--- Query 1:
    ...
    FROM music_lesson
    WHERE EXTRACT(YEAR FROM music_lesson.date) = '2023'
    ...
    -- retrieve the average number of lessons per month during 2023.
...
--- Query 2:
    ...
    FROM (SELECT COUNT(student_id) AS siblings FROM sibling GROUP BY student_id) AS x
    GROUP BY siblings ORDER BY siblings;
    
    SELECT COUNT(*) AS students, siblings
    FROM (SELECT student_id, SUM(CASE WHEN student_id IN (SELECT id FROM student) THEN 1 ELSE 0 END) AS siblings
    ...
    FROM (SELECT count(sibling.student_id) as total 
        FROM student LEFT JOIN sibling ON student.id = sibling.student_id  
        GROUP BY student.id) as number_of_siblings 
    GROUP BY total_siblings ORDER BY total_siblings ASC;

    --query
        SELECT * FROM student_number_of_siblings;
...
--- Query 3:
    ...
    SELECT * FROM 
        (SELECT COUNT(*) AS number_of_lessons, instructor_id AS instructor FROM music_lesson
    ...
    SELECT * FROM 
        (SELECT COUNT(*) AS number_of_lessons, instructor_id AS instructor, lesson_type FROM music_lesson
        WHERE TO_CHAR(date, 'YYYY-MM') = TO_CHAR(current_date, 'YYYY-MM')
...
--- Query 4:
    ...
    --materialized view
    CREATE MATERIALIZED VIEW number_of_ensembles AS
	SELECT music_lesson.lesson_type AS type, music_lesson.date AS date_of_lesson , ensemble.genre AS genre,
        (CASE 
	        WHEN COUNT (student_ensemble.student_id) = (maximum_number_of_students) THEN 'Fully booked'
	        WHEN COUNT (student_ensemble.student_id) = (maximum_number_of_students - 1) THEN '1 seats left'
	...
	GROUP BY type , date_of_lesson , genre, maximum_number_of_students;
    --query 
    SELECT * FROM number_of_ensembles;
